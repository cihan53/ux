...
const selectedColumns = computed(() => props.columns.map(col => {

  if (props.filterBar) {
    return {
      ...col,
      width: col.width ? `${Math.max(parseInt(col.width), 250)}px` : '200px',
...
const thClass = computed(() => {
  return {
    ...
    th: {
      base: `text-left rtl:text-right ${props.filterBar ? 'border border-gray-200 dark:divide-gray-700 dark:border-gray-700' : 'border border-gray-200 dark:border-gray-700'}`,
...
<template>
  <div> 
    <UTable v-model="selection" :rows="resolvedData" :loading="loading || internalLoading" :columns="selectedColumns.filter(col => !col.hidden)" :progress="progress"
      :loading-state="loadingState" :ui="{
        wrapper: 'min-h-full',
        ...thClass,
      ...
      <!-- Filter Bar -->
      <template v-for="col in columns" :key="col.key" #[`${col.key}-header`]="{ column, sort, onSort }">
        <ClientOnly>
          <div class="flex flex-col group" :style="{ minWidth: column.width }" v-if="props.filterBar">
            <div
              class="w-full pl-3 divide-x divide-gray-200 border-b border-gray-200 dark:divide-gray-700 dark:border-gray-700  flex items-center justify-between gap-1">
              <span class="text-sm py-2">{{ column.label }}</span>
              <UDropdown class="opacity-0 group-hover:opacity-100" :items="headerMenuItems"
    ...
    <div class="flex items-end gap-1">
      <div class="ml-auto flex items-center gap-4">
        <div class="flex items-center gap-2">
          <UPagination v-if="paginationConfig.enabled" v-model="currentPage" :page-count="paginationConfig.pageSize"
            :total="totalCount" :show-first="paginationConfig.showFirst" :show-last="paginationConfig.showLast"
            :ui="{ wrapper: 'mt-4' }" />
        </div>
...
<style scoped>
button[role="menuitem"]:hover>.dropdown-content {
  display: block;
}
</style>